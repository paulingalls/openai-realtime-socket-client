...
import { hasNativeWebSocket, trimDebugEvent } from './utils.js';
import {
...
export class RealtimeVoiceClient extends EventEmitter<RealtimeVoiceEvents> {
  ...
  protected saveSession(newSession: RealtimeSession) {
    ...
    console.log('Saving session:', sessionCopy);
  ...
  protected receive(type: string, message: any) {
    switch (type) {
      ...
      case 'conversation.item.deleted':
        ...
        this.emit('conversation.item.deleted', message);
        break;
      case 'response.output_item.added':
        this.transcription.addItem(message.item, message.previous_item_id);
        this.emit('response.output_item.added', message);
        break;
      case 'response.output_item.done':
        this.transcription.updateItem(message.item.id, message.item);
        this.emit('response.output_item.done', message);
        break;
      default:
        // @ts-ignore
